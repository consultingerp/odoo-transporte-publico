...
class registro_empresa(models.Model):
    ...
    conductor_id = fields.One2many('registro.conductor', 'conductor_id', 'Conductores', select=True, track_visibility='onchange', required=True)
    vehiculo_id = fields.One2many('registro.vehiculo', 'vehiculo_id', 'Vehículos', select=True, track_visibility='onchange', required=True)